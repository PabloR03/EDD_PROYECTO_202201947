...
contains
    ...
    subroutine top_5_imagenes(self)
        ...
        print *, "ID Imagen: ", trim_m(id1), "con un total De Capas:  ", trim_m(max1)
        print *, "ID Imagen: ", trim_m(id2), "con un total De Capas:  ", trim_m(max2)
        print *, "ID Imagen: ", trim_m(id3), "con un total De Capas:  ", trim_m(max3)
        print *, "ID Imagen: ", trim_m(id4), "con un total De Capas:  ", trim_m(max4)
        print *, "ID Imagen: ", trim_m(id5), "con un total De Capas:  ", trim_m(max5)
    ...
    subroutine graficar_arbol(this,nombre_grafica)
        ...
        codigo_dot = codigo_dot // "node [shape=Mcircle];" // new_line('a')
        codigo_dot = codigo_dot //&
        'Titulo [fontname="Comic Sans MS", color=red shape=component label="Arbol Binario AVL"]' &
        // new_line('a')
    ...
    recursive subroutine RoamTree(actual, crear_nodo, direccion_nodo)
            ...
            crear_nodo = crear_nodo // '"' // trim(direccion) // '"' // '[fontname="Comic Sans MS" label="' &
    ...
    subroutine graficar_arbol_imagen(this, nombre_grafica, valor_destacado)
        ...
        direccion_nodo = ''
        codigo_dot = "digraph G{" // new_line('a')
        codigo_dot = codigo_dot // "node [shape=Msquare];" // new_line('a')
        codigo_dot = codigo_dot //&
        'Titulo [fontname="Comic Sans MS", color=blue shape=component label="Arbol Binario AVL Imagen"]' &
        // new_line('a')
        codigo_dot = codigo_dot // "{rank=same; Titulo;}" // new_line('a')
        if (associated(this%raiz)) then
    ...
    subroutine generar_grafica(nombre_grafica, codigo)
        ...
        dot_nombre_grafica = "gg/" // trim(nombre_grafica) // ".dot"
        png_nombre_grafica = "gg/" // trim(nombre_grafica) // ".png"
        filepath = 'gg/' // trim(nombre_grafica) 
