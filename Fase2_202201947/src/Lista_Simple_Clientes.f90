...
contains
    subroutine insertar_cliente(self, dpi, nombre, contrasena)
        ...
        if (existe) then
            print*, "Usuario: ",trim(dpi)," Ya Esta Registrado, Verifique su contrasena."
            return
        end if
        ...
        print*, "Usuario: ",trim(dpi)," Registrado Correctamente, proceda a iniciar sesion."
    ...
    subroutine modificar_cliente(self, dpi, nombre, contrasena)
        ...
        existe = self%cliente_existe(dpi)
        if (.not. existe) then
            print*, "Cliente: ",trim(dpi)," No Esta Registrado, Utilice la opcion de registro."
            return
        end if
        actual => self%cabeza
        do while (associated(actual))
    ...
    subroutine grafica_cliente(self, filename)
        ...
        filepath = 'gg/' // trim(filename) 
        open(unit, file=filepath, status='replace')
        write(unit, *) 'digraph cola {node [fontname="Comic Sans MS"]'
        write(unit, *) '    node [shape=Msquare, style=filled, color=purple, fillcolor="#3d5a80"];'
        actual => self%cabeza
        contador = 0
        write(unit, *) '"Node',contador,'"[shape=folder, color=black, fillcolor="#65babf" label="',"Lista Clientes",'"];'
        do while (associated(actual))
            contador = contador + 1
            write(unit, *) '    "Node', contador, '" [label="', &
                                "DPI: ", trim(actual%dpi), "\n", &
                                "Nombre: ", trim(actual%nombre), "\n", &
                                "Contrasena: ",trim(actual%contrasena),'"];'
