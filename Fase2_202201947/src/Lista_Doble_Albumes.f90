module Mod_ListaAlbumes
    ...
    subroutine eliminar_imagen(self, imagen)
        ...
        if (.not. associated(self%cabeza)) then
            print*,"No existe una lista de Albumes."
            return
        end if
    ...
    subroutine graficar_album(self, filename)
        ...
        write(unit, *) 'node [shape=folder, style=filled, color=blue, fillcolor="#3d5a80"];'
        actual => self%cabeza
        contador = 0
        write(unit, *) '"Node',contador,'" [shape=doubleoctagon, color=black, fillcolor="#65babf" label="',"Lista Albumes",'"];'
        do while (associated(actual))
            ...
            if (associated(actual_imagen)) then
                ...
                '" [shape=component, color=black, fillcolor="#52E7FE" label="',actual_imagen%imagen,'"];'
