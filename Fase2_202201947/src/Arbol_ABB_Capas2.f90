...
contains
    ...
    subroutine generar_grafica(self, crear_nodo, enlace_nodo, direccion_nodo_avl)
        ...
        if (associated(self%raiz)) then
            ...
            crear_nodo = crear_nodo // '"' // trim(direccion_abb) // '"' // '[fontname="Comic Sans MS" label="' &
            // trim(str_valor) // '", color=purple];' // new_line('a')
